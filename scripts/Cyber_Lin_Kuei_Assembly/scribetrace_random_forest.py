...
def load_training_arrays(path: Path):
    ...
    Returns:
        tuple[np.ndarray, np.ndarray, list[str], list[str]]: Feature matrix,
        class labels, image paths, and the ordered feature schema.
    """
    ...
    return features, labels, image_paths, feature_names
...
def train_random_forest(dataset_jsonl: Path):
    ...
    X, y, image_paths, feature_names = load_training_arrays(dataset_jsonl)
    ...
    sample_indices = np.arange(dataset_row_count, dtype=np.int32)
    try:
        ...
        raise ValueError(
            "Stratified train/validation/test split failed. Increase "
...
def main():
    ...
    if args.mode == "export":
        ...
        dataset_path = export_scribetrace_dataset(limit_per_class=limit_per_class)
